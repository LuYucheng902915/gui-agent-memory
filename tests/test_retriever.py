...
class TestMemoryRetriever:
    ...
    def retriever(self, mock_config, mock_storage):
        ...
        with patch('gui_agent_memory.retriever.get_config', return_value=mock_config), \
             patch('gui_agent_memory.retriever.MemoryStorage', return_value=mock_storage):
    ...
    def test_retrieve_memories(self, retriever, mock_config, mock_storage):
        ...
        mock_response = Mock()
        mock_response.data = [Mock(embedding=[0.1, 0.2, 0.3])]
        mock_config.get_embedding_client.return_value.embeddings.create.return_value = mock_response
        
        # Mock storage query results
        mock_storage.query_experiences.return_value = {
            "ids": [["exp_1"]],
            "documents": [["Test experience"]],
            "metadatas": [[{"keywords": ["test"], "is_successful": True, "source_task_id": "test_1", "preconditions": "None", "action_flow": "[]"}]],
            "distances": [[0.1]]
        }
