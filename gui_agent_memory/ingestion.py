...
class MemoryIngestion:
    ...
    def _setup_logging(self) -> None:
        ...
        os.makedirs(os.path.dirname(self.config.failed_learning_log_path), exist_ok=True)
    ...
    def _extract_keywords_with_jieba(self, text: str) -> List[str]:
        ...
        stop_words = {'的', '是', '在', '和', '与', '或', '但是', '如果', '那么', 'the', 'a', 'an', 'and', 'or', 'but', 'if', 'then'}
        keywords = [token.lower() for token in tokens if len(token) > 1 and token.lower() not in stop_words]
    ...
    def _distill_experience_with_llm(self, learning_request: LearningRequest) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=self.config.experience_llm_model,
                messages=[
                    {"role": "system", "content": "You are an expert at analyzing GUI automation tasks."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.2
            )
        ...
        try:
            ...
            for step_data in distilled_data.get("action_flow", []):
                action_step = ActionStep(
                    ...
                    target_element_description=step_data.get("target_element_description", "")
                )
            ...
            experience = ExperienceRecord(
                task_description=distilled_data.get("task_description", task_description),
                keywords=keywords,
