...
class MemoryRetriever:
    ...
    def _extract_query_keywords(self, query: str) -> List[str]:
        ...
        stop_words = {
            '的', '是', '在', '和', '与', '或', '但是', '如果', '那么', '了', '也', '就', '都', '要', '可以', '这', '那',
            'the', 'a', 'an', 'and', 'or', 'but', 'if', 'then', 'to', 'for', 'with', 'by', 'from', 'at', 'on', 'in'
        }
            ...
            if 'results' in api_response:
                ...
                for i, result_data in enumerate(api_response['results'][:top_n]):
                    ...
                    if doc_index < len(candidates):
                        ...
                        result['rerank_score'] = result_data.get('relevance_score', 1.0 - i * 0.1)
